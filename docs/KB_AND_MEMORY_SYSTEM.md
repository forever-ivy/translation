...
### 2) openclaw-mem（记忆）
openclaw-mem 是**跨任务的短文本记忆**（类似“决策摘要/约束摘要”），用于：

- 在新任务开始时，按 query 召回相关记忆（`openclaw memory search`）；
- 在任务结束后，把“真正的术语/风格决策点”写入记忆（`openclaw memory store`）。

记忆的目标：在“KB 文件检索未命中/命中不稳/任务描述很短”的情况下，仍能提供关键约束。
...
Knowledge Repository/
  00_Glossary/          # 术语表（通用）
  10_Style_Guide/       # 风格指南（通用）
  20_Domain_Knowledge/  # 领域知识（通用）
...
2) `retrieve_kb_with_fallback(...)`：
   - 优先 clawrag（若启用且命中可用）
   - 否则 fallback 到本地 BM25/FTS（或进一步 fallback 到 token-count 方案）
3) `knowledge_context`（hits 列表）写入 `meta`，作为 `openclaw_translation_orchestrator.run()` 的输入上下文。

### 4.2 “多公司并行但暂不做隔离”的当前实现
当前通过 `OPENCLAW_KB_ISOLATION_MODE` 控制 reference 的过滤策略（默认 `reference_only`），并依赖 `jobs.kb_company`：

- `reference_only`（默认）：只对 `30_Reference/` 做公司隔离；同时允许全局目录（`00/10/20/40`）参与检索。
- `all`：所有 KB 命中都必须在 `30_Reference/{Company}/...` 下（更强隔离）。

公司选择：
- `run`/`rerun` 时若 `kb_company` 为空，会列出 `KB/30_Reference/*` 作为菜单，用户数字选择后继续执行（见 `scripts/skill_approval.py`）。
...
## 6. 记忆系统：从质量报告/变更日志抽取“真正决策”写入 openclaw-mem
